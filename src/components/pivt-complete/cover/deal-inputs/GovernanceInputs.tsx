...
import { Upload, FileText, CheckCircle2, Clock, ShieldCheck, Loader2 } from 'lucide-react';
...
const GOV_DOC_VALUES = GOV_DOC_TYPES.map(t => t.value);
...
export const GovernanceInputs: React.FC = () => {
  ...
  // Auto-populate from contract_documents with governance doc types
  useEffect(() => {
    ...
    const fetchDocs = async () => {
      ...
      const { data } = await supabase
        ...
        .eq('deal_id', dealId)
        .in('doc_type', GOV_DOC_VALUES as any);
    ...
    <div className="space-y-8">
      <motion.div {...fadeInUp}>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Corporate approvals required before closing.
          {docs.length > 0 && !isDemoDeal && <span className="text-accent"> Auto-populated from uploaded approval documents.</span>}
        </p>
      </motion.div>
                ...
                {docs.map(doc => (
                  <tr key={doc.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-2.5">
                      <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2 py-1 rounded-full ${doc.status === 'VERIFIED' || doc.status === 'EXTRACTION_COMPLETE' ? 'bg-emerald-500/10 text-emerald-600' : 'bg-muted/60 text-muted-foreground'}`}>
                        {doc.status === 'VERIFIED' || doc.status === 'EXTRACTION_COMPLETE' ? <CheckCircle2 className="w-3 h-3" /> : <Clock className="w-3 h-3" />}{doc.status}
                      </span>
                    </td>
