...
interface ContractDocument {
  ...
  deal_id: string;
  doc_type: string;
  document_category: 'FINANCIAL_STRUCTURING' | 'CONTRACTUAL_OBLIGATION';
  filename: string;
...
const FINANCIAL_DOC_TYPES = [
  ...
  { value: 'DISTRIBUTION_SCHEDULE', label: 'Distribution Schedule' },
  { value: 'STRUCTURING_OTHER', label: 'Other Structuring Input' },
] as const;
...
const CONTRACT_DOC_TYPES = [
  ...
  { value: 'CONTRACT_OTHER', label: 'Other Contract Doc' },
] as const;
...
// ── Demo Data ──
const DEMO_FINANCIAL_DOCS: ContractDocument[] = [
  { id: 'fd1', deal_id: 'atlas', doc_type: 'CAP_TABLE', document_category: 'FINANCIAL_STRUCTURING', filename: 'Atlas_CapTable_Final.xlsx', file_url: null, status: 'PARSED', uploaded_at: '2026-02-25T09:00:00Z', text_content: null },
  { id: 'fd2', deal_id: 'atlas', doc_type: 'WATERFALL_MODEL', document_category: 'FINANCIAL_STRUCTURING', filename: 'Waterfall_Schedule_v3.xlsx', file_url: null, status: 'PARSED', uploaded_at: '2026-02-26T14:00:00Z', text_content: null },
];

const DEMO_CONTRACT_DOCS: ContractDocument[] = [
  { id: 'd1', deal_id: 'atlas', doc_type: 'SPA', document_category: 'CONTRACTUAL_OBLIGATION', filename: 'Atlas_SPA_v3_Final.pdf', file_url: null, status: 'EXTRACTION_COMPLETE', uploaded_at: '2026-02-28T10:30:00Z', text_content: 'Stock Purchase Agreement...' },
  { id: 'd2', deal_id: 'atlas', doc_type: 'FUNDS_FLOW', document_category: 'CONTRACTUAL_OBLIGATION', filename: 'FundsFlow_Atlas_v2.pdf', file_url: null, status: 'EXTRACTION_COMPLETE', uploaded_at: '2026-02-28T11:00:00Z', text_content: 'Funds Flow Memo...' },
  { id: 'd3', deal_id: 'atlas', doc_type: 'ESCROW_AGREEMENT', document_category: 'CONTRACTUAL_OBLIGATION', filename: 'Escrow_Agreement_JPMorgan.pdf', file_url: null, status: 'UPLOADED', uploaded_at: '2026-03-01T09:00:00Z', text_content: null },
];
...
// ── Main Component ──
export const DealInputsCover: React.FC = () => {
  const [financialDocs, setFinancialDocs] = useState<ContractDocument[]>(DEMO_FINANCIAL_DOCS);
  const [contractDocs, setContractDocs] = useState<ContractDocument[]>(DEMO_CONTRACT_DOCS);
  const [obligations, setObligations] = useState<Obligation[]>(DEMO_OBLIGATIONS);
  const [selectedFinType, setSelectedFinType] = useState('CAP_TABLE');
  const [selectedContractType, setSelectedContractType] = useState('SPA');
  ...
  const handleFinancialUpload = useCallback(() => {
    const newDoc: ContractDocument = {
      id: `fd${Date.now()}`,
      deal_id: deal.id,
      doc_type: selectedFinType,
      document_category: 'FINANCIAL_STRUCTURING',
      filename: `${deal.codeName}_${selectedFinType}_${new Date().toISOString().slice(0, 10)}.xlsx`,
      file_url: null,
      status: 'UPLOADED',
  ...
  const handleContractUpload = useCallback(() => {
    const newDoc: ContractDocument = {
      id: `d${Date.now()}`,
      deal_id: deal.id,
      doc_type: selectedContractType,
      document_category: 'CONTRACTUAL_OBLIGATION',
      filename: `${deal.codeName}_${selectedContractType}_${new Date().toISOString().slice(0, 10)}.pdf`,
      file_url: null,
      status: 'UPLOADED',
      uploaded_at: new Date().toISOString(),
      text_content: null,
  ...
  const handleExtract = useCallback(async (docId: string) => {
    ...
    setTimeout(() => {
      ...
      const newObs: Obligation[] = [{
        id: `ob-${Date.now()}-1`, deal_id: deal.id, source_document_id: docId,
        obligation_type: 'OTHER', status: 'NEEDS_REVIEW', timing_type: 'AT_CLOSING',
        scheduled_date: null, payor_label: 'Buyer', payee_label: 'TBD',
        amount_type: 'UNKNOWN', amount_value_minor: null, amount_currency: 'USD',
        percent_basis_points: null, percent_base_reference: null, confidence_score: 0.65,
        source_text_snippet: 'Extracted from document...', extracted_by: 'AI',
        confirmed_by_user_id: null, confirmed_at: null, mapping_status: 'UNMAPPED',
  ...
  const stats = useMemo(() => ({
    ...
    unmapped: obligations.filter(o => o.mapping_status === 'UNMAPPED' && o.status === 'CONFIRMED').length,
    readinessScore: Math.round((obligations.filter(o => o.status === 'CONFIRMED').length / Math.max(obligations.length, 1)) * 100),
  }), [obligations]);

  const exportCSV = useCallback(() => {
    const headers = ['Type', 'Status', 'Payor', 'Payee', 'Amount', 'Currency', 'Timing', 'Confidence', 'Source Doc'];
    const rows = obligations.map(o => [
      OBLIGATION_TYPE_LABELS[o.obligation_type] || o.obligation_type, o.status,
      o.payor_label || '', o.payee_label || '', formatAmount(o), o.amount_currency || '',
      TIMING_LABELS[o.timing_type] || o.timing_type, o.confidence_score.toFixed(2),
      contractDocs.find(d => d.id === o.source_document_id)?.filename || '',
    ]);
    const csv = [headers.join(','), ...rows.map(r => r.map(c => `"${c}"`).join(','))].join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `obligations_${deal.codeName}_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click(); URL.revokeObjectURL(url);
    toast.success('Obligations exported');
  }, [obligations, contractDocs, deal]);

  const getDocTypeLabel = (type: string) => {
    return [...FINANCIAL_DOC_TYPES, ...CONTRACT_DOC_TYPES].find(t => t.value === type)?.label || type;
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div {...fadeInUp} className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold" style={{ letterSpacing: '-0.03em' }}>Deal Inputs</h2>
          <p className="text-sm text-muted-foreground mt-1">
            Structured intake for financial models and transaction documents.
          </p>
        </div>
      ...
      {/* ══════════ SECTION A: Financial Structuring Inputs ══════════ */}
      <motion.div {...fadeInUp} className="pivt-card overflow-hidden">
        <div className="p-5 border-b border-border/30">
          ...
          <p className="text-xs text-muted-foreground ml-8">
            Upload spreadsheets used to structure ownership and distributions.
          </p>
        </div>
          ...
          <div className="flex items-end gap-4 mb-5">
            <div className="flex-1">
              ...
              <select
                ...
                onChange={e => setSelectedFinType(e.target.value)}
            ...
            <Button onClick={handleFinancialUpload} className="gap-1.5">
              ...
              {financialDocs.map(doc => (
                <tr key={doc.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-2.5">
                    <Badge variant="outline" className="text-xs">{getDocTypeLabel(doc.doc_type)}</Badge>
                  </td>
                  <td className="px-4 py-2.5">
                    <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2 py-1 rounded-full ${
                      doc.status === 'PARSED' ? 'bg-emerald-500/10 text-emerald-600' :
                      doc.status === 'ERROR' ? 'bg-red-400/10 text-red-400' :
                      'bg-muted/60 text-muted-foreground'
                    }`}>
                      {doc.status === 'PARSED' && <CheckCircle2 className="w-3 h-3" />}
              ...
              {financialDocs.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-muted-foreground text-sm">
                    No financial inputs uploaded yet.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {/* ══════════ SECTION B: Contract Inputs (Obligations) ══════════ */}
      <motion.div {...fadeInUp} className="pivt-card overflow-hidden">
        <div className="p-5 border-b border-border/30">
          <div className="flex items-center gap-3 mb-1">
            <FileText className="w-5 h-5 text-accent" />
            <h3 className="font-semibold">Contract Inputs</h3>
            <div className="flex-1" />
            {/* Execution Readiness Badge */}
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium ${
              stats.readinessScore >= 80 ? 'bg-emerald-500/10 text-emerald-600' :
              stats.readinessScore >= 50 ? 'bg-amber-500/10 text-amber-600' :
              'bg-red-400/10 text-red-400'
            }`}>
              <div className={`w-2 h-2 rounded-full ${
                stats.readinessScore >= 80 ? 'bg-emerald-500' :
                stats.readinessScore >= 50 ? 'bg-amber-500' : 'bg-red-400'
              }`} />
              Execution Readiness: {stats.readinessScore}%
            </div>
          </div>
          <p className="text-xs text-muted-foreground ml-8">
            Upload transaction documents to extract contractual obligations for closing.
          </p>
        </div>
          ...
          <div className="flex items-end gap-4">
            <div className="flex-1">
              ...
              <select
                ...
                className="w-full bg-muted/30 border border-border/50 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-accent/40"
              >
                {CONTRACT_DOC_TYPES.map(t => (
                  <option key={t.value} value={t.value}>{t.label}</option>
                ))}
              </select>
            </div>
            <Button onClick={handleContractUpload} className="gap-1.5">
                ...
                <tr key={doc.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-2.5">
                    ...
                    {doc.status === 'EXTRACTION_COMPLETE' && (
                      <Button size="sm" variant="ghost" className="gap-1.5 text-xs" onClick={() => setShowObligations(true)}>
                        <Eye className="w-3 h-3" /> View Obligations
                      </Button>
                    )}
                  </td>
                </tr>
        ...
        {/* ── Obligations Sub-Panel ── */}
        {(showObligations || obligations.some(o => o.status !== 'REJECTED')) && (
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-4 gap-3">
                {[
                  ...
                  { label: 'Unmapped', value: stats.unmapped, icon: Link2, color: stats.unmapped > 0 ? 'text-red-400' : undefined },
                ].map(s => (
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {['all', 'NEEDS_REVIEW', 'DRAFT_EXTRACTED', 'CONFIRMED', 'REJECTED'].map(s => (
                  <button key={s} onClick={() => setStatusFilter(s)}
                    className={`px-2.5 py-1 rounded-full text-xs font-medium transition-all ${
                      statusFilter === s ? 'bg-accent/10 text-accent border border-accent/30' : 'bg-muted/30 text-muted-foreground hover:bg-muted/50 border border-transparent'
                    }`}
                  >
                ...
                <div className="flex-1" />
                <button onClick={() => setSortByConfidence(!sortByConfidence)}
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-all ${
                    sortByConfidence ? 'bg-amber-500/10 text-amber-600 border border-amber-500/30' : 'bg-muted/30 text-muted-foreground border border-transparent'
                  }`}
                >
                  <Shield className="w-3 h-3" /> Low confidence first
                </button>
              </div>

              {/* Obligations Table + Drawer */}
              <div className="flex gap-4">
                <div className={`overflow-hidden rounded-lg border border-border/30 ${selectedObligation ? 'flex-1' : 'w-full'}`}>
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border/30 bg-muted/20">
                        ...
                        <th className="text-left px-3 py-2.5 text-xs font-medium text-muted-foreground uppercase tracking-wider">Payee</th>
                        <th className="text-left px-3 py-2.5 text-xs font-medium text-muted-foreground uppercase tracking-wider">Amount</th>
                        <th className="text-left px-3 py-2.5 text-xs font-medium text-muted-foreground uppercase tracking-wider">Timing</th>
                        <th className="text-left px-3 py-2.5 text-xs font-medium text-muted-foreground uppercase tracking-wider">Conf.</th>
                        <th className="text-left px-3 py-2.5 text-xs font-medium text-muted-foreground uppercase tracking-wider">Mapping</th>
                    ...
                    <tbody>
                      {filteredObligations.map(ob => {
                        ...
                        return (
                          <tr key={ob.id}
                            className={`border-b border-border/20 hover:bg-muted/20 transition-colors cursor-pointer ${selectedObligation?.id === ob.id ? 'bg-accent/5' : ''}`}
                            onClick={() => setSelectedObligation(ob)}
                          >
                            <td className="px-3 py-2.5 font-medium text-xs">{OBLIGATION_TYPE_LABELS[ob.obligation_type] || ob.obligation_type}</td>
                            <td className="px-3 py-2.5">
                              <span className={`inline-flex items-center text-[11px] font-medium px-2 py-0.5 rounded-full ${style.bg} ${style.text}`}>{ob.status.replace(/_/g, ' ')}</span>
                            </td>
                            <td className="px-3 py-2.5 text-xs">{ob.payee_label || '—'}</td>
                            <td className="px-3 py-2.5 text-xs font-mono">{formatAmount(ob)}</td>
                            <td className="px-3 py-2.5 text-xs text-muted-foreground">{TIMING_LABELS[ob.timing_type] || ob.timing_type}</td>
                            <td className="px-3 py-2.5">
                              <span className={`text-xs font-mono font-medium ${confidenceColor(ob.confidence_score)}`}>{(ob.confidence_score * 100).toFixed(0)}%</span>
                            </td>
                            <td className="px-3 py-2.5">
                              <Badge variant="outline" className={`text-[10px] ${ob.mapping_status === 'MAPPED' ? 'border-emerald-500/30 text-emerald-600' : 'border-muted'}`}>
                                {ob.mapping_status === 'MAPPED' && <Link2 className="w-2.5 h-2.5 mr-0.5" />}
                                {ob.mapping_status}
                              </Badge>
                            </td>
                            <td className="px-3 py-2.5">
                                ...
                                {(ob.status === 'NEEDS_REVIEW' || ob.status === 'DRAFT_EXTRACTED') && (
                                  <>
                                    <Tooltip><TooltipTrigger asChild>
                                      <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => handleConfirm(ob.id)}>
                                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                                      </Button>
                                    </TooltipTrigger><TooltipContent>Confirm</TooltipContent></Tooltip>
                                    <Tooltip><TooltipTrigger asChild>
                                      <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => handleReject(ob.id)}>
                      ...
                      {filteredObligations.length === 0 && (
                        <tr><td colSpan={8} className="px-4 py-8 text-center text-muted-foreground text-sm">No obligations match the current filter.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
                ...
                <AnimatePresence>
                  {selectedObligation && (
                    <motion.div initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 360 }} exit={{ opacity: 0, width: 0 }} className="pivt-card overflow-hidden shrink-0">
                      <div className="p-5 space-y-4 h-full overflow-y-auto">
                        <div className="flex items-center justify-between">
                          <h3 className="text-sm font-semibold">Obligation Detail</h3>
                          <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => setSelectedObligation(null)}><X className="w-3.5 h-3.5" /></Button>
                        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs">{contractDocs.find(d => d.id === selectedObligation.source_document_id)?.filename || 'Unknown'}</p>
                          </div>
                        ...
                        {(selectedObligation.status === 'NEEDS_REVIEW' || selectedObligation.status === 'DRAFT_EXTRACTED') && (
                          <div className="flex gap-2 pt-2 border-t border-border/30">
                            <Button size="sm" className="flex-1 gap-1.5" onClick={() => handleConfirm(selectedObligation.id)}>
                              <CheckCircle2 className="w-3.5 h-3.5" /> Confirm
                            </Button>
                            <Button size="sm" variant="outline" className="flex-1 gap-1.5" onClick={() => handleReject(selectedObligation.id)}>
                        ...
                        {selectedObligation.mapping_status === 'UNMAPPED' && selectedObligation.status === 'CONFIRMED' && (
                          <Button size="sm" variant="outline" className="w-full gap-1.5" onClick={() => toast.info('Map to Intent UI coming soon')}>
                            <Link2 className="w-3.5 h-3.5" /> Map to Disbursement Intent
                          </Button>
                        )}
                      </div>
                    </motion.div>
        ...
        {obligations.length > 0 && (
          <div className="border-t border-border/30 p-5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-sm font-semibold">Execution Readiness</h3>
              <div className="flex-1" />
              <span className={`text-sm font-mono font-semibold ${
                stats.readinessScore >= 80 ? 'text-emerald-600' : stats.readinessScore >= 50 ? 'text-amber-600' : 'text-red-400'
              }`}>{stats.readinessScore}%</span>
            </div>
            <div className="grid grid-cols-4 gap-4">
              {[
                { label: 'Total Obligations', value: stats.total, ok: true },
                { label: 'Confirmed', value: stats.confirmed, ok: stats.confirmed === stats.total },
                { label: 'Unmapped', value: stats.unmapped, ok: stats.unmapped === 0 },
                { label: 'Needs Review', value: stats.needsReview, ok: stats.needsReview === 0 },
              ].map(item => (
                <div key={item.label} className="flex items-center gap-3 p-3 rounded-lg bg-muted/20">
                  <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${item.ok ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                  <div>
